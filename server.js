// DeepRWA — Full backend: auth, chat, persistence, versions, sharing
...
} catch (e) { console.warn('Brevo init failed:', e.message); }
...
async function sendEmailCode(toEmail, code, purpose = 'verification') {
  ...
  sendSmtpEmail.htmlContent = `
    <div style="font-family:Inter,sans-serif;max-width:480px;margin:0 auto;padding:32px;background:#0f1115;color:#e6e8eb;border-radius:12px;">
      ...
      <p style="color:#5c636d;font-size:12px;margin:24px 0 0;">This code expires in 15 minutes.</p>
    </div>`;
  sendSmtpEmail.sender = { name: 'DeepRWA', email: process.env.EMAIL_FROM || 'noreply@deeprwa.agentdomains.co' };
  sendSmtpEmail.to = [{ email: toEmail }];
  try { await brevoClient.sendTransacEmail(sendSmtpEmail); return true; }
  catch (err) { console.error('Brevo error:', err.message); return false; }
}
  ...
  try {
    const res = await fetch('https://api.groq.com/openai/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'Generate a short 2-6 word title for this chat. Reply with only the title — no quotes, no prefix like "Title:", no ending punctuation.' },
    ...
    title = title.replace(/^["'`]+|["'`]+$/g, '').replace(/^Title:\s*/i, '').split('\n')[0].trim();
    if (!title || title.length > 60) title = firstMessage.split(/\s+/).slice(0, 5).join(' ');
    return title || 'New chat';
  } catch {
    return firstMessage.split(/\s+/).slice(0, 5).join(' ') || 'New chat';
  }
}

// ============ ROUTES ============
app.get('/health', (req, res) => res.json({ status: 'ok', service: 'DeepRWA', version: '2.1.0', time: new Date().toISOString() }));

app.get('/api/config', (req, res) => res.json({
  name: 'DeepRWA', tagline: 'Your AI guide to Rwanda', version: '2.1.0',
...
app.post('/api/auth/login', async (req, res) => {
  ...
  const code = Math.floor(100000 + Math.random() * 900000).toString();
  const pendingToken = jwt.sign({ type: 'login', userId: user.id, email: user.email, code }, process.env.JWT_SECRET, { expiresIn: '15m' });
...
app.post('/api/chat', requireAuth, async (req, res) => {
  ...
  if (!convId) {
    const firstText = messages.find(m => m.role === 'user')?.content || 'New chat';
...
app.get('/api/share/:token', async (req, res) => {
  ...
  const { data: sharedMsg } = await supabase.from('shared_messages').select('message_id').eq('token', token).maybeSingle();
  if (sharedMsg) {
    const { data: msg } = await supabase.from('messages').select('*').eq('id', sharedMsg.message_id).single();
    return res.json({ type: 'message', messages: msg ? [msg] : [] });
  }
